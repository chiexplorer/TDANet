...
def build_norm_layer(norm_cfg, num_features):
    ...
    Args:
        norm_cfg (dict): The config dict for normalization layer.
        num_features (int): Number of features in the normalization layer.

    Returns:
        nn.Module: Created normalization layer.
    """
    layer_type = norm_cfg['type']
...
class ConvModule(nn.Module):
    def __init__(self, in_chans, embed_dim, kernel_size, stride=1, padding='auto', groups=1, bias='auto', dim=2,
                 norm_cfg="BN2d", act_cfg=None, order: tuple = ('conv', 'norm', 'act')):
        ...
        self.with_bias = bias
        self.dim = dim
        padding = kernel_size // 2 if padding == 'auto' else padding
        # 创建卷积层
        if dim == 2:
            self.conv = nn.Conv2d(in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding,
                                  groups=groups, bias=bias)
        elif dim == 1:
            self.conv = nn.Conv1d(in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding,
                                  groups=groups, bias=bias)

        # 根据 norm_cfg 创建归一化层
        if norm_cfg is not None:
            if norm_cfg['type'] == 'BN2d' and dim == 2:  # 批量归一化
                self.norm = nn.BatchNorm2d(embed_dim)
            elif norm_cfg['type'] == 'LN':  # LayerNorm
                self.norm = nn.LayerNorm(embed_dim)
            elif norm_cfg['type'] == "GN":  # GroupNorm
                self.norm = GlobLN(embed_dim)
            else:
        ...
        if act_cfg is not None:
            if act_cfg['type'] == 'ReLU':
                ...
                raise NotImplementedError(f"Activation function {act_cfg['type']} is not implemented")
        else:
            self.act = None
        self.init_weights()
    ...
    def forward(self, x, activate=True, norm=True):
        x = self.conv(x)
        if self.norm is not None and norm and self.with_norm:
            x = self.norm(x)
        if self.act is not None and activate:
            x = self.act(x)
        return x

    def init_weights(self):
        if not hasattr(self.conv, 'init_weights'):
            if self.act is not None and isinstance(self.act, nn.LeakyReLU):
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.act = nn.ReLU() if (act_cfg is not None) else nn.Identity()
        self.norm = build_norm_layer(norm_layer, embed_dim)
...
class Attention1D(nn.Module):  ### OSRA
    def __init__(self, dim,
                 num_heads=1,
                 qk_scale=None,
                 attn_drop=0,
...
class CrossAttention1D(nn.Module):  ### CrossOSRA
    def __init__(self, dim,
                 num_heads=1,
                 qk_scale=None,
                 attn_drop=0,
    ...
    def forward(self, x, context=None, relative_pos_enc=None):
        B, C, L = x.shape
...
class DynamicConv2d(nn.Module):  ### IDConv
    def __init__(self,
        ...
        self.proj = nn.Sequential(
            ConvModule(dim,
                       dim // reduction_ratio,
                       kernel_size=1,
                       norm_cfg=dict(type='BN2d'),  # 原 BN2d
                       act_cfg=dict(type='GELU'), ),
    ...
    def forward(self, x):
        ...
        weight = weight.reshape(-1, 1, self.K, self.K)  # dynamic kernel weight
...
class DynamicConv1d(nn.Module):  ### IDConv
    def __init__(self,
                 dim,
                 kernel_size=3,
                 reduction_ratio=4,
...
class Mlp1D(nn.Module):  ### MS-FFN 1D
    """
    Mlp implemented by with k=1 convolutions.

    Input: Tensor with shape [B, C, L].
    Output: Tensor with shape [B, C, L].
    Args:
        in_features (int): Dimension of input features.
        hidden_features (int): Dimension of hidden features.
        out_features (int): Dimension of output features.
        ...
        if self.fork_feat:
            ...
            for i_emb, i_layer in enumerate(self.out_indices):
                if i_emb < start_level:
                    ...
                    layer = build_norm_layer(norm_cfg, embed_dims[(i_layer + 1) // 2])
...
if __name__ == '__main__':
    ...
    # # # TransXNet测试
    # model = TransXNet(arch='t',  image_size=224,
    #              norm_cfg=dict(type='GN', num_groups=1),
    #              act_cfg=dict(type='GELU'),
    #              in_chans=3,
    #              in_patch_size=7,
    ...
    # # # IDConv测试
    # module = DynamicConv2d(64, 3, 4, 2, bias=False).cuda()
    # # module = nn.Conv2d(64, 64, 3, padding=1, groups=64).cuda()
    # x = torch.rand(1, 64, 128, 128, device=device)
    # macs, params = profile(module, inputs=(x, ))
    # mb = 1000*1000
    # print(f"MACs: [{macs/mb/1000}] Gb \nParams: [{params/mb}] Mb")
    # print("模型参数量详情：")
    # summary(module, input_size=(1, 64, 128, 128), mode="train")
    # y = module(x)
    # print(y.shape)
    ...
    # # OSRA测试
    # dim = 64
    # module = Attention(dim,
    #              num_heads=1,
    #              qk_scale=None,
    #              attn_drop=0,
    #              sr_ratio=4,).cuda()
    # x = torch.rand(1, dim, 128, 128, device=device)
    # macs, params = profile(module, inputs=(x, ))
    # mb = 1000*1000
    # print(f"MACs: [{macs/mb/1000}] Gb \nParams: [{params/mb}] Mb")
    # print("模型参数量详情：")
    ...
    # # OSRA 1d 测试
    # dim = 64
    # module = Attention1D(dim,
    #              num_heads=1,
    #              qk_scale=None,
    #              attn_drop=0,
    #              sr_ratio=4,).cuda()
    # x = torch.rand(1, dim, 1500, device=device)
    # macs, params = profile(module, inputs=(x, ))
    # mb = 1000*1000
    # print(f"MACs: [{macs/mb/1000}] Gb \nParams: [{params/mb}] Mb")
    # print("模型参数量详情：")
    ...
    # # # MS-FFn测试
    # feat_len = 64
    # module = Mlp(in_features=feat_len,
    #     hidden_features = None,
    #     out_features = None,
    #     act_cfg = dict(type='GELU'),
    #     drop = 0,).cuda()
    ...
    # # # MS-FFn 1维测试
    # feat_len = 64
    # module = Mlp1D(in_features=feat_len,
    #     hidden_features = None,
    #     out_features = None,
    #     act_cfg = dict(type='GELU'),
    #     drop = 0,).cuda()
